...
export const book_created = firestore.document('{mode}/{book_id}/{type}/{id}').onCreate(async function (snap, { params }) {
  let message;
  console.log(params);
...
export const book_deleted = firestore.document('{mode}/{book_id}/{type}/{id}').onDelete(function (snap, { params }) {
  console.log(params);
  console.log(snap.data());
...
export const chat_updated = firestore.document('{mode}/{book_id}/chats/{id}').onUpdate(function ({ before, after }, { params }) {
  let book_id, mode;
  console.log(params);
...
export const game_updated = firestore.document('game/{book_id}').onUpdate(async function ({ before, after }, { params }) {
  const { locale_id } = after.data();
  const { book_id } = params;
  const idx = await next_idx(admin.firestore().collection(`game/${book_id}/parts`));
  ...
  const npc_ref = await admin.firestore().doc(`game/${book_id}/potofs/${book_id}-NPC`).get();
